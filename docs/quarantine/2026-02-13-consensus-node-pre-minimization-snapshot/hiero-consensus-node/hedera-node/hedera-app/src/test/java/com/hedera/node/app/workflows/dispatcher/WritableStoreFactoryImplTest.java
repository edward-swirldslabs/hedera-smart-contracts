...
class WritableStoreFactoryImplTest {
    ...
    @Test
    void contractServiceCanAccessClprWritableStores() {
        given(stack.getWritableStates(ContractService.NAME)).willReturn(writableStates);
        given(stack.getWritableStates(ClprService.NAME)).willReturn(writableStates);
        final WritableStoreFactory subject =
                new WritableStoreFactory(stack, ContractService.NAME, mock(WritableEntityCounters.class));

        final var queueStore = subject.getStore(WritableClprMessageQueueMetadataStore.class);
        final var messageStore = subject.getStore(WritableClprMessageStore.class);

        assertThat(queueStore).isInstanceOf(WritableClprMessageQueueMetadataStore.class);
        assertThat(messageStore).isInstanceOf(WritableClprMessageStore.class);
    }
}
