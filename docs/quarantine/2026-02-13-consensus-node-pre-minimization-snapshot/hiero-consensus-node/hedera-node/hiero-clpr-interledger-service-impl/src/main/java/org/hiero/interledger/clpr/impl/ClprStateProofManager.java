...
public class ClprStateProofManager {
    /**
     * CLPR ledger ids are consumed as {@code bytes32} in EVM route headers and system-contract payloads.
     * In dev mode, we normalize any non-empty id to 32 bytes via SHA-256.
     */
    private static final int LEDGER_ID_BYTES = 32;
     ...
     *
     * <p><b>Dev Mode Behavior:</b> Uses CLPR metadata when present; otherwise derives from the active roster hash.
     * If neither source is available yet an empty {@link ClprLedgerId} is returned so callers can treat the
    ...
    public ClprLedgerId getLocalLedgerId() {
        ...
        if (metadata != null && metadata.ledgerId() != null && metadata.ledgerId().ledgerId().length() > 0) {
            return normalizeLedgerId(metadata.ledgerId());
        }
        final var rosterLedgerId = deriveLedgerIdFromRosterHash(snapshot);
        if (rosterLedgerId != null) {
            return rosterLedgerId;
        }
        return emptyLedgerId();
    }

    @NonNull
    private static ClprLedgerId normalizeLedgerId(@NonNull final ClprLedgerId ledgerId) {
        final var normalized = normalizeLedgerIdBytes(ledgerId.ledgerId());
        if (ledgerId.ledgerId().equals(normalized)) {
            return ledgerId;
        }
        return ClprLedgerId.newBuilder().ledgerId(normalized).build();
    }

    @NonNull
    private static Bytes normalizeLedgerIdBytes(@NonNull final Bytes rawLedgerIdBytes) {
        if (rawLedgerIdBytes.length() == 0) {
            return Bytes.EMPTY;
        }
        if (rawLedgerIdBytes.length() == LEDGER_ID_BYTES) {
            return rawLedgerIdBytes;
        }
        return Bytes.wrap(sha256(rawLedgerIdBytes.toByteArray()));
    }
    ...
    @Nullable
    private ClprLedgerId deriveLedgerIdFromRosterHash(@NonNull final BlockProvenSnapshot snapshot) {
        try {
            final var rosterStates = snapshot.state().getReadableStates(RosterStateId.SERVICE_NAME);
            if (!rosterStates.contains(RosterStateId.ROSTER_STATE_STATE_ID)) {
                return null;
            }
            final var rosterHash = new ReadableRosterStoreImpl(rosterStates).getCurrentRosterHash();
            if (rosterHash == null || rosterHash.length() == 0) {
                return null;
            }
            return ClprLedgerId.newBuilder()
                    .ledgerId(normalizeLedgerIdBytes(rosterHash))
                    .build();
