...
public class ClprEndpointClient {
    ...
    void runOnce() {
        try {
            ...
            if (localConfig == null) {
                final var seeded = seedMissingLocalConfiguration(localLedgerId, clprConfig.publicizeNetworkAddresses());
                if (seeded) {
                    log.info("CLPR endpoint maintenance seeded local configuration for ledger {}", localLedgerId);
                } else {
            ...
            for (final var entry : configsByLedgerId.entrySet()) {
                ...
                // Native-queue integrations and EVM route headers consume ledger ids as bytes32. Ignore any legacy
                // configurations that are not bytes32 so dev-mode migrations don't accidentally treat them as remotes.
                if (remoteLedgerId.ledgerId().length() != 32) {
                    log.debug(
                            "CLPR endpoint maintenance skipping non-bytes32 remote ledger id {} (len={})",
                            remoteLedgerId,
                            remoteLedgerId.ledgerId().length());
                    continue;
                }
    ...
    /**
     * Seeds local CLPR configuration if bootstrap dispatch did not persist it.
     *
     * <p>This is a dev-mode resilience path for local/SOLO environments where control-plane startup can leave
     * CLPR metadata initialized but local configuration absent.</p>
     */
    private boolean seedMissingLocalConfiguration(
            @NonNull final ClprLedgerId localLedgerId, final boolean includeServiceEndpoint) {
        final var selfNode = networkInfo.selfNodeInfo();
        final var selfAccount = selfNode.accountId();
        final var localEndpoint = localServiceEndpoint();
        ServiceEndpoint advertisedEndpoint = localEndpoint;
        if (includeServiceEndpoint) {
            final var candidate = resolveAdvertisedServiceEndpoint(selfNode, localEndpoint.port());
            if (candidate != null) {
                advertisedEndpoint = candidate;
            }
        }
        final var localConfig =
                bootstrapLocalConfiguration(localLedgerId, selfAccount, advertisedEndpoint, includeServiceEndpoint);
        final var proof = buildLocalClprStateProofWrapper(localConfig);
        try (final ClprClient localClient = connectionManager.createClient(localEndpoint)) {
            final var status = localClient.setConfiguration(selfAccount, selfAccount, proof);
            if (isSuccessful(status)) {
                return true;
            }
            log.warn(
                    "CLPR endpoint maintenance failed to seed local configuration for ledger {} (status={})",
                    localLedgerId,
                    status);
            return false;
        } catch (final Exception e) {
            log.warn("CLPR endpoint maintenance failed to seed local configuration for ledger {}", localLedgerId, e);
            return false;
        }
    }

    private ClprLedgerConfiguration bootstrapLocalConfiguration(
        ...
        final var endpointBuilder =
                ClprEndpoint.newBuilder().nodeAccountId(selfAccount).signingCertificate(Bytes.EMPTY);
        if (includeServiceEndpoint) {
            endpointBuilder.endpoint(localEndpoint);
        }
        return ClprLedgerConfiguration.newBuilder()
                .ledgerId(localLedgerId)
                .timestamp(toTimestamp(clock.instant()))
                .endpoints(java.util.List.of(endpointBuilder.build()))
                .build();
    }

    private @Nullable ServiceEndpoint resolveAdvertisedServiceEndpoint(
            @NonNull final com.hedera.node.app.spi.info.NodeInfo selfNode, final int primaryGrpcPort) {
        final var endpoints = selfNode.hapiEndpoints();
        if (endpoints == null || endpoints.isEmpty()) {
            return null;
        }
        for (final var endpoint : endpoints) {
            if (endpoint != null && endpoint.port() == primaryGrpcPort) {
                return endpoint;
            }
        }
        // Fall back to the first endpoint if no exact match is found.
