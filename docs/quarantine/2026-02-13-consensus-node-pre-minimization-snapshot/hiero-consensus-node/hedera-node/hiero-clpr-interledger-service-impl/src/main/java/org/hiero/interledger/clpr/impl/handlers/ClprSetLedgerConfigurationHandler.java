...
public class ClprSetLedgerConfigurationHandler implements TransactionHandler {
    ...
    private void pureChecks(@NonNull final TransactionBody txn, final boolean validateStateProof)
        ...
        final var allowDevModeLocalBootstrapBypass = validateStateProof
                && stateProofManager.isDevModeEnabled()
                && isNodeBootstrapSubmission(txn);
        // Extract and validate the configuration from the state proof.
        final ClprLedgerConfiguration ledgerConfig = extractConfigurationOrThrow(
                configTxnBdy, localLedgerId, validateStateProof, allowDevModeLocalBootstrapBypass);
    ...
    private boolean isNodeBootstrapSubmission(@NonNull final TransactionBody txn) {
        if (!txn.hasTransactionID()) {
            return false;
        }
        final var payer = txn.transactionIDOrThrow().accountID();
        if (payer == null) {
            return false;
        }
        try {
            final var selfInfo = networkInfo.selfNodeInfo();
            if (selfInfo != null && payer.equals(selfInfo.accountId())) {
                return true;
            }
            final var addressBook = networkInfo.addressBook();
            if (addressBook != null) {
                for (final var nodeInfo : addressBook) {
                    if (nodeInfo != null && payer.equals(nodeInfo.accountId())) {
                        return true;
                    }
                }
            }
        } catch (final RuntimeException ignore) {
            // If node roster/account lookup is unavailable during bootstrap, keep strict proof validation.
        }
        return false;
    }
    ...
    private ClprLedgerConfiguration extractConfigurationOrThrow(
            @NonNull final ClprSetLedgerConfigurationTransactionBody txn,
            @NonNull final org.hiero.hapi.interledger.state.clpr.ClprLedgerId localLedgerId,
            final boolean validateStateProof,
            final boolean allowDevModeLocalBootstrapBypass)
            throws PreCheckException {
        try {
            ...
            if (validateStateProof && !ClprStateProofUtils.validateStateProof(stateProof)) {
                final var ledgerId = extractedConfig.ledgerIdOrThrow();
                final var existingConfig = stateProofManager.readLedgerConfiguration(ledgerId);
                final var allowBypass = allowDevModeLocalBootstrapBypass
                        && ledgerId.equals(localLedgerId)
                        && existingConfig == null;
                validateTruePreCheck(allowBypass, ResponseCodeEnum.CLPR_INVALID_STATE_PROOF);
            }
