...
package com.hedera.node.app.service.contract.impl.test.exec.systemcontracts.clpr.enqueuemessageresponse;
...
import com.hedera.node.app.service.contract.impl.exec.systemcontracts.clpr.enqueuemessageresponse.ClprQueueEnqueueMessageResponseTranslator;
...
class ClprQueueEnqueueMessageResponseTranslatorTest extends CallAttemptTestBase {
    ...
    void selectorMatchesAdrLock() {
        assertThat(subject.identifyMethod(attemptWithInput(Bytes.fromHexString("b26aa82b")))).isPresent();
        assertThat(subject.identifyMethod(attemptWithInput(Bytes.fromHexString("8cfaaa60")))).isEmpty();
    }
    ...
    void responsePathEnqueuesReplyAndReturnsAssignedId() {
        ...
        final var call = subject.callFrom(attemptWithInput(validResponseInput((byte) 1, REMOTE_LEDGER_ID, originalMessageId)));
        final var result = call.execute(frame);
        ...
        verify(messageStore).put(any(), valueCaptor.capture());
        verify(queueStore).put(any(), any());
        assertThat(valueCaptor.getValue().payloadOrThrow().messageReplyOrThrow().messageId())
    ...
    private Bytes validResponseInput(final byte routeVersion, final byte[] routeLedgerId, final long originalMessageId) {
        final var routeHeaderBuffer = ROUTE_HEADER_TYPE.encode(Tuple.of(
                Byte.toUnsignedInt(routeVersion), routeLedgerId, asHeadlongAddress(OWNER_BESU_ADDRESS)));
